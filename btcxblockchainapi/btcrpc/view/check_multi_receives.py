...
class CheckMultiAddressesReceive(APIView):
    ...
    if post_serializers.is_valid():
      ...
      try:
        for transaction in transactions:
          ...
          response = self.create_receive_information_response_and_log(log_info,
        ...
        receives_response = self.create_receives_information_response_and_log(log_info,
        ...
        receives_response = self.create_receives_information_response_and_log(log_error,
                                                                              error_message,
                                                                              ex,
                                                                              response_list=response_list,
                                                                              chain=chain,
  ...
  def __get_txIds(self, rpc_service, account=""):
    ...
    log_info(log, "transactions received of category receive for the specific account " + account, transactions_with_tx_id)
    return transactions_with_tx_id

  def create_receive_information_response_and_log(self, log_function, log_message, log_item, currency, address, received,
                                                  risk, txs):
    log_function(log, log_message, log_item)
    receive_info_response = check_multi_receives.ReceiveInformationResponse(currency=currency,
                                                    address=address,
                                                    received=received,
                                                    risk=risk,
                                                    txs=txs)
